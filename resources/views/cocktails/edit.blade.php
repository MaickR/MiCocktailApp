...
<form method="POST" action="{{ route('cocktails.update', $cocktail->id) }}">
    ...
    <div class="mb-3">
        <label for="alcoholic" class="form-label">Tipo (Alcoholic / Non alcoholic)</label>
        <input type="text" class="form-control" name="alcoholic" value="{{ old('alcoholic', $cocktail->alcoholic) }}">
    </div>
